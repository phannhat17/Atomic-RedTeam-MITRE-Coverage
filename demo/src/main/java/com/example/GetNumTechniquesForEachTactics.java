...
public class GetNumTechniquesForEachTactics 
{
    ...
    // List of tactics
    private final String[] tacticsList = {
            "collection", "command-and-control", "credential-access", "defense-evasion", "discovery", "execution", "exfiltration", "impact",
    ...
    private boolean isValidAttackPattern(JSONObject obj, String tactic) 
    {
        if (obj.getString("type").equals("attack-pattern")
                && !obj.optBoolean("x_mitre_is_subtechnique")
                && !obj.optBoolean("x_mitre_deprecated", false)
                && !obj.optBoolean("revoked", false)) 
        {
